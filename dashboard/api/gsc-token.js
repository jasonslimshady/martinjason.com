// ============================================================
//  /api/gsc-token — persistent Google Search Console login
//
//  Mints Search Console access tokens from a stored OAuth refresh
//  token so the dashboard never has to show the Google login popup
//  again. Same architecture as /api/ga-token.js, kept as a fully
//  separate route + separate app_tokens row ('gsc') so this can
//  never interfere with the existing GA4 connection.
//
//  One-time setup:
//    1. No new SQL needed — app_tokens already exists (see
//       supabase-setup.sql, used by GA4 under id='ga4').
//    2. In Google Cloud Console, enable the "Search Console API"
//       for the same OAuth client already used for GA4/Gmail, and
//       (if the OAuth consent screen is in Testing mode) add the
//       webmasters.readonly scope to it.
//    3. Reuses GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET /
//       SUPABASE_SERVICE_ROLE_KEY — already set in Vercel for GA4.
//    4. Call gscConnect() from the dashboard (console or a future
//       button) once — the refresh token is stored and every
//       device stays connected from then on.
//
...
const TOKEN_ID         = 'gsc'; // row id in the app_tokens table — separate from 'ga4'
  ...
  try {
    if (req.method === 'POST') {
      ...
      if (tokens.refresh_token) {
        ...
        if (!up.ok) {
          throw new Error('Refresh-Token konnte nicht gespeichert werden — app_tokens-Tabelle vorhanden?');
        }
      }
    ...
    let refreshToken = process.env.GSC_REFRESH_TOKEN || null;
    if (!refreshToken) {
      const q = await fetch(
    ...
    try {
      ...
      if (e.google === 'invalid_grant') {
        ...
        return res.status(409).json({
          error: 'Google-Verbindung (Search Console) abgelaufen — bitte einmal neu verbinden',
    ...
    console.error('gsc-token error:', err.message);
